  ...
  asyncHandler(async (req, res) => {
    try {
      const coupons = await Coupon.findAll({
        include: [
          {
            model: Category, // Модель для `applicableCategory`
            as: "applicableCategory", // Псевдоним ассоциации, если используется
            attributes: ["id", "name"], // Выбор полей
          },
          {
            model: SubCategory, // Модель для `applicableSubCategory`
            as: "applicableSubCategory", // Псевдоним ассоциации
            attributes: ["id", "name"], // Выбор полей
          },
          {
            model: Product, // Модель для `applicableProduct`
            as: "applicableProduct", // Псевдоним ассоциации
            attributes: ["id", "name"], // Выбор полей
          },
        ],
      });
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      const updatedCoupon = await Coupon.update(
        ...
        { where: { id: couponID }, returning: true }
      );
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    try {
      const couponID = req.params.id;
      const deletedCoupon = await Coupon.findByPk(couponID);
      if (!deletedCoupon) {
        return res
          .status(404)
          .json({ success: false, message: "Coupon not found." });
