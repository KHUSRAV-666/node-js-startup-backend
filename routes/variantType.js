...
router.get(
  ...
  asyncHandler(async (req, res) => {
    try {
      ...
      const variantType = await VariantType.findByPk(variantTypeID);
      if (!variantType) {
        return res
          .status(404)
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      const updatedVariantType = await VariantType.update(
        { name, type },
        { where: { id: variantTypeID }, returning: true }
      );
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const variantCount = await Variant.count({
        where: {
          variantTypeId: variantTypeID, // Условие для фильтрации по variantTypeId
        },
      });
      if (variantCount > 0) {
